...
## Objectif
Préparer l'image Docker qui sera utilisée par Kubernetes dans l'exercice 07.

À la fin de l'exercice :

- l'image `devops-app:1.0.0` existe localement
- l'application a été testée dans un conteneur Docker simple
- l'image a été chargée dans le cluster local `minikube` ou `kind`
- Kubernetes pourra créer les pods sans `ImagePullBackOff`

## Contexte
L'exercice 06 a terminé le cycle Terraform + Ansible sur des conteneurs Docker locaux.

Avant de migrer vers Kubernetes, il faut figer l'application sous forme d'image. Kubernetes ne construit pas l'image pour nous dans ce TP : il consomme une image déjà disponible dans son runtime.

Le fil rouge devient donc :

```text
06  : Terraform + Ansible configurent les conteneurs Docker
06b : Docker construit l'image applicative
07  : Terraform déploie cette image dans Kubernetes
```

## Pré-requis

- Docker opérationnel
- le dossier `starter-code/app`
